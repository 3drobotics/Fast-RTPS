    ...
    for (ReliabilityQosPolicyKind qos : {BEST_EFFORT_RELIABILITY_QOS, RELIABLE_RELIABILITY_QOS}) {
        for (int pubHist : {histSmall, histLarge}) {
            for (int subHist : {histSmall, histLarge}) {
                ...
                if (qos == RELIABLE_RELIABILITY_QOS) {
                    if (msgCount < pubHist && msgCount > subHist) {
                        //
                        // XXX FAILURE XXX
                        // subCount is probably zero, which is wrong, and the
                        // subscriber will never get another message
                        //
                        // The reader history ends up with a few of the most
                        // recent changes (but with gaps, e.g. 93 94 96 98 100
                        // where 100 is newest), and the reader's writer proxy
                        // has a changesFromWLowMark_ of 68, so the "take"
                        // will always fail because change 68 will never
                        // arrive. The fix might be to, when the old change is
                        // deleted to make room for a new change, update the
                        // writerproxy such that it knows the old sample will
                        // never be received.
                    } else if (msgCount < pubHist && msgCount < subHist) {
